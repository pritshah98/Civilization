...
public class GameController {
    ...
    private static boolean move(TerrainTile start, TerrainTile end) {
        if (start.getOccupant().getOwner()
            ...
            if (!(end.isEmpty())) {
                ...
                errorAudio.play();
                Text moveError = new Text("Cannot move onto occupied space.");
                error.getDialogPane().setContent(moveError);
                error.showAndWait();
            }
        }
        ...
        AudioClip walking =
            new AudioClip("File:./src/main/java/audio/Walking.mp3");
        walking.play();
        state = GameState.NEUTRAL;
        return true;
    }
    ...
    private static void attackTile(TerrainTile attacker, TerrainTile enemy) {
        if (attacker.getOccupant().getOwner()
            == GameController.getCivilization()) {
            if (enemy.getOccupant().isFriendly()) {
                error.setTitle("ERROR");
                AudioClip errorAudio =
                    new AudioClip("File:./src/main/java/audio/Error.mp3");
                errorAudio.play();
                Text attackError = new Text("Cannot attack friendly unit.");
                error.getDialogPane().setContent(attackError);
                error.showAndWait();
            }
        }
        if (enemy.isEmpty()
        ...
        AudioClip attack =
            new AudioClip("File:./src/main/java/audio/Attack.mp3");
        attack.play();
